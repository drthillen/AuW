...
class Main {
    ...
    public static HashSet<Integer> DFS(LinkedList<Integer>[] vertices) {
        ...
        for (int u = 0; u < V; u++) {
            if (!visited[u]) {
                currDFS = DFSVisit(u, -1, low, dfs, pred, vertices, visited, currDFS, currDFS, articV) + 1;
            }
        }
    ...
    public static int DFSVisit(int u, int pre, int[] low, int[] dfs, int[] pred, LinkedList<Integer>[] vertices,
            boolean[] visited, int startDFS, int currDFS, HashSet<Integer> articV) {
        visited[u] = true;
        dfs[u] = currDFS;
        low[u] = currDFS;
        pred[u] = pre;
        ...
        for (int v : vertices[u]) {
            if (!visited[v]) {
                ...
                currDFS = DFSVisit(v, u, low, dfs, pred, vertices, visited, startDFS, currDFS + 1, articV);
